...
框架先产生叶子文档，再汇聚为 PDSCH、PHY 和仓库级文档。叶子文档与父级汇总统一采用九章契约：模块概述、领域背景、系统交互关系、任务流程、核心实现、状态与时序、调试与故障定位、设计经验、Agent 开发导航。证据限制和待确认内容写入相关章节正文，不额外增加同级章节。

统一的是章节入口，不是内容深度：叶子文档保留源码实现细节；父级文档把直接子文档作为主要证据，只汇聚跨子模块关系、公共约束、主流程和下钻导航。框架会补全直接子文档链接，并拒绝写入机械复制子文档长段落的层级汇总。

入库时，生成快照会在运行目录的 `knowledge/documents/modules/<module-id>/` 下按模块单独保存；仓库级文档保存到 `knowledge/documents/repository/`。数据库文档记录和检索切块同时保留 `module_id`、`module_folder` 与 `storage_path`，因此不同信道或子模块不会混在一个平铺目录中。
