...
ClangWiki 是一套面向大型 C/C++ 代码仓的**本地代码知识工作流**。它以 Clang/`libclang` 提取的代码事实为基础，按“配置的模块根目录下一层子模块”为最小叶子文档单元，自底向上生成 Wiki；同时管理多个仓库、逻辑知识空间、代码关系图、人工知识和有引用的 RAG 问答，为 ChatGPT 或其他 Coding Agent 提供可检索、可理解、可验证的代码知识上下文。
...
- 编译器证据代码图谱：构建目标、翻译单元、仓库、模块、文件、符号、领域概念和 Wiki 多层可视化；候选关系与确定关系严格分离。
