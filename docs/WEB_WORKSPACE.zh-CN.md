...
“生成/更新完整 Wiki”用于首次生成和全仓更新。“选择性文档维护”默认收起，仅在需要重做指定文档类型或模块时使用。生成过程中，界面会展示分析、上下文构建、OpenCode 调用、格式校验、写入和断点复用等阶段；失败时保留具体错误、标准输出与错误日志位置。

每篇文档成功写入后都会记录断点。失败、取消或意外中断的任务可选择“继续”，复用已完成文档并从剩余任务开始；“重来”则从头执行。源码、配置或文档规范改变后，旧断点会被判定为不兼容，不能继续。

Wiki 页面将层级模块文档按对应源码目录展示。用户可以导出单篇 Markdown，或将当前快照完整导出为保持目录层级的 ZIP。需要让文档随仓库固定保存时，可显式点击“同步到代码仓”，把当前有效快照写入 `<代码仓>/docs/clangwiki/`；该操作会弹出确认，且只更新 ClangWiki 管理清单记录的文件，不会修改源码或删除未知人工文件。未点击同步时，内部快照仍只保存在 ClangWiki 数据根目录。
