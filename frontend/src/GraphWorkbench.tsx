...
export default function GraphWorkbench({ scope, notify }: { scope: Scope; notify: (text: string, error?: boolean) => void }) {
  ...
  const [insights, setInsights] = useState<{ hub: any[]; bridge: any[]; orphan: any[]; cycle: any[] }>({ hub: [], bridge: [], orphan: [], cycle: [] });
  const [insight, setInsight] = useState<"community" | "hub" | "bridge" | "orphan" | "cycle">("community");
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setGraph(value); setDiagnostics(value.diagnostics || null); setSelected(null); setSelectedEdge(null); setDetail(null); setPath(null);
  ...
  useEffect(() => { setLevel(preset.level); setFocusedCommunity(null); }, [view]);
  useEffect(() => { void load(); }, [load]);
  useEffect(() => {
    if (!scope || scope.type !== "repository") { setCommunities([]); setInsights({ hub: [], bridge: [], orphan: [], cycle: [] }); return; }
    Promise.all([
      ...
      setCommunities(a.communities || []); setInsights({ hub: b.nodes || [], bridge: c.nodes || [], orphan: d.nodes || [], cycle: e.cycles || [] });
    }).catch((error) => notify(message(error), true));
  ...
  return <section className="kg-workbench">
    <aside className="kg-navigator">
      <div className="kg-search"><Search size={14} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="搜索节点、路径或符号" /></div>
      ...
      {scope.type === "repository" && <>
        <div className="kg-insight-tabs">{(["community", "hub", "bridge", "cycle", "orphan"] as const).map((item) => <button key={item} className={insight === item ? "active" : ""} onClick={() => setInsight(item)}>{({ community: "社区", hub: "核心", bridge: "桥梁", cycle: "循环", orphan: "孤点" } as any)[item]}</button>)}</div>
    ...
    <div className="kg-main">
      ...
      <div className="kg-toolbar">
        <div className="kg-levels">{(["repository", "module", "file", "symbol"] as Level[]).map((item) => <button className={level === item ? "active" : ""} key={item} onClick={() => setLevel(item)}>{({ repository: "仓库", module: "模块", file: "文件", symbol: "符号" } as any)[item]}</button>)}</div>
    ...
    <aside className={`kg-inspector ${activeItem ? "open" : ""}`}>
      {selected ? <>
        ...
        <dl><Info label="模块" value={selected.module_id || "—"} /><Info label="源码" value={selected.path || "—"} /><Info label="位置" value={selected.line_start ? `${selected.line_start}-${selected.line_end || selected.line_start}` : "—"} /><Info label="社区" value={selected.community_id?.split(":").pop() || "—"} /><Info label="入度 / 出度" value={`${selected.metrics?.in_degree || 0} / ${selected.metrics?.out_degree || 0}`} /><Info label="介数中心性" value={Number(selected.metrics?.betweenness || 0).toFixed(4)} /></dl>
