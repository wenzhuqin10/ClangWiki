  ...
  return <div className="app-shell">
    ...
    <main className="workspace">
      ...
      <div className="view-container">
        ...
        {view === "repositories" && <Repositories repositories={repositories} scope={scope} moduleTree={moduleTree} modelOptions={status?.model_options || []} select={chooseRepository} onAdd={() => setModal("repo")} onGenerate={() => runAction("generate")} onIndex={() => runAction("index")} onJobs={() => setView("jobs")} notify={notify} refresh={refresh} />}
...
function Repositories({ repositories, scope, moduleTree, modelOptions, select, onAdd, onGenerate, onIndex, onJobs, notify, refresh }: any) {
  ...
  const resume = async () => { if (!selected || !resumableRun) return; try { await api.post(`/api/repositories/${selected.id}/runs/${resumableRun.id}/resume`); notify("已从最近断点继续生成；已完成文档不会再次调用模型。"); onJobs(); } catch (error) { notify(message(error), true); } };
