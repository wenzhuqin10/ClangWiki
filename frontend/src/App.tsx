...
function Wiki({ scope, onManual, notify }: { scope: Scope; onManual: () => void; notify: Function }) {
  ...
  return <div className="wiki-layout"><aside className="document-sidebar"><div className="document-tools"><div className="input-with-icon"><Search size={15} /><input value={filter} onChange={(e) => setFilter(e.target.value)} placeholder="筛选 Wiki 页面" /></div><button className="icon-button" onClick={onManual} title="新建人工知识页"><Plus size={16} /></button></div><div className="document-list">{shown.map((doc) => <button key={doc.id} className={selected?.id === doc.id ? "selected" : ""} onClick={() => load(doc.id)}><FileText size={15} /><span><strong>{doc.title}</strong><small>{doc.kind === "manual" ? "人工知识" : `${documentRoleLabel(doc.document_role)} · ${doc.relative_path}`}</small></span>{doc.kind === "manual" && <Tags size={13} />}</button>)}{!shown.length && <Empty icon={<BookOpen />} title="还没有 Wiki" text="生成仓库或集合 Wiki 后将在这里显示。" />}</div></aside><article className="markdown-panel">{selected ? <><div className="document-meta"><div><span className="eyebrow">{selected.kind === "manual" ? "MANUAL KNOWLEDGE" : "GENERATED SNAPSHOT"}</span><h2>{selected.title}</h2></div><div><span className="evidence-badge"><CheckCircle2 size={13} />{selected.kind === "manual" ? "工程师知识" : documentRoleLabel(selected.document_role)}</span></div></div><div className="markdown-body"><ReactMarkdown remarkPlugins={[remarkGfm]}>{selected.content || ""}</ReactMarkdown></div></> : <Empty icon={<BookOpen />} title="选择一篇文档" text="文档内容、源码证据与版本信息会显示在这里。" />}</article></div>;
}
