...
function App() {
  ...
  return <div className="app-shell">
    ...
    {modal === "repo" && <RepositoryModal close={() => setModal(null)} done={async (repo: any) => { setModal(null); await refresh(); chooseRepository(repo); notify("代码仓已注册，源码未被复制或修改。"); }} />}
...
function Jobs({ jobs, notify, refresh }: any) {
  ...
  return <section className="card jobs-card"><CardTitle title="全部任务" eyebrow={`${jobs.length} JOBS`} /><div className="jobs-table"><div className="table-head"><span>任务</span><span>范围</span><span>状态</span><span>进度</span><span>操作</span></div>{jobs.map((job: any) => <div className="job-line" key={job.id}><span><JobIcon status={job.status} /><span><strong>{jobTitle(job.kind)}</strong><small>{job.id}</small></span></span><code>{job.scope_id || "系统"}</code><StatusBadge status={job.status} /><span><i className="progress"><b style={{ width: `${job.progress || 0}%` }} /></i><em>{Math.round(job.progress || 0)}%</em></span><span>{["queued", "running"].includes(job.status) ? <button className="mini-button" onClick={() => act(job, "cancel")}>取消</button> : job.status === "failed" ? <button className="mini-button" onClick={() => act(job, "retry")}>重试</button> : "—"}</span></div>)}{!jobs.length && <Empty icon={<Activity />} title="暂无任务记录" text="分析、生成和索引任务会持久化保存在这里。" />}</div></section>;
}

function SystemSettings({ status }: any) {
  return <div className="settings-grid"><section className="card"><CardTitle title="本地运行环境" eyebrow="RUNTIME" /><div className="health-list"><Health name="ClangWiki 服务" ok={Boolean(status)} detail={status ? `v${status.version}` : "未连接"} /><Health name="OpenCode CLI" ok={Boolean(status?.opencode)} detail={status?.opencode || "未在 PATH 中发现"} /><Health name="FastEmbed" ok={Boolean(status?.vector_runtime?.fastembed?.available)} detail={status?.vector_runtime?.fastembed?.available ? status.vector_runtime.fastembed.version : "可选组件，当前使用降级检索"} /><Health name="USearch" ok={Boolean(status?.vector_runtime?.usearch?.available)} detail={status?.vector_runtime?.usearch?.available ? status.vector_runtime.usearch.version : "可选组件，当前使用降级检索"} /></div></section><section className="card"><CardTitle title="数据与安全边界" eyebrow="LOCAL ONLY" /><DescriptionList rows={[["数据根目录", status?.data_root || "—"], ["监听范围", "127.0.0.1（仅本机）"], ["模型调用", "opencode run"], ["API Key", "不接收、不保存、不读取"], ["源码权限", "只读；删除注册不会删除源码"], ["前端资源", "随 Python 包离线分发"]]} /></section><section className="card full-span"><CardTitle title="离线 Embedding 配置" eyebrow="OPTIONAL VECTOR CHANNEL" /><div className="profile-grid">{Object.entries(status?.embedding_profiles || {}).map(([key, value]: [string, any]) => <div key={key}><span className="profile-icon"><Database /></span><div><strong>{key === "balanced" ? "平衡档" : "高质量档"}</strong><code>{value.model}</code><p>{value.dimension} 维 · CPU ONNX 本地推理</p></div>{key === "balanced" && <span className="recommended">默认</span>}</div>)}</div></section></div>;
}

function RepositoryModal({ close, done }: any) {
  const [form, setForm] = useState({ path: "", name: "", model: "", channels: "" }); const [busy, setBusy] = useState(false); const [error, setError] = useState("");
  const submit = async (event: FormEvent) => { event.preventDefault(); setBusy(true); setError(""); try { const config: any = {}; if (form.model) config.model = form.model; if (form.channels) config.channel_module_paths = form.channels.split(",").map((v) => v.trim()).filter(Boolean); done(await api.post("/api/repositories", { path: form.path, name: form.name || null, config })); } catch (e) { setError(message(e)); } finally { setBusy(false); } };
  return <Modal title="注册本地代码仓" subtitle="ClangWiki 只保存路径和分析产物，不复制或修改源码。" close={close}><form onSubmit={submit} className="form"><label><span>仓库绝对路径</span><input required autoFocus value={form.path} onChange={(e) => setForm({ ...form, path: e.target.value })} placeholder="D:\projects\pdsch-channel" /></label><label><span>显示名称（可选）</span><input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="PDSCH 信道仓" /></label><label><span>OpenCode 模型标识</span><input value={form.model} onChange={(e) => setForm({ ...form, model: e.target.value })} placeholder="企业配置中的 provider/glm-5.1" /><small>这里只保存模型 ID，不接受 API Key。</small></label><label><span>信道根（逗号分隔，可选）</span><input value={form.channels} onChange={(e) => setForm({ ...form, channels: e.target.value })} placeholder="src/phy/pdsch, src/phy/pusch" /><small>其下一层源码目录将作为最小叶子文档单元。</small></label>{error && <div className="form-error">{error}</div>}<div className="modal-actions"><button type="button" className="button ghost" onClick={close}>取消</button><button className="button primary" disabled={busy}>{busy && <LoaderCircle className="spin" size={15} />}注册仓库</button></div></form></Modal>;
}
