...
import {
  ...
  CircleHelp, Clock3, Code2, Database, Download, FileCode2, FileText, Folder, FolderGit2, GitBranch,
  ...
  return <div className="app-shell">
    ...
    <main className="workspace">
      ...
      <div className="view-container">
        ...
        {view === "repositories" && <Repositories repositories={repositories} scope={scope} moduleTree={moduleTree} jobs={jobs} modelOptions={status?.model_options || []} select={chooseRepository} onAdd={() => setModal("repo")} onGenerate={() => runAction("generate")} onIndex={() => runAction("index")} onJobs={() => setView("jobs")} notify={notify} refresh={refresh} />}
...
function DocumentGenerationPanel({ repositoryId, moduleTree, notify, onJobs }: any) {
  ...
  return <section className={`card document-generation-card ${expanded ? "expanded" : "collapsed"}`}>
    <CardTitle title="选择性文档维护" eyebrow="ADVANCED DOCUMENT MAINTENANCE" action={<div className="maintenance-actions"><span className="doc-status">已有 {generatedCount} 篇快照</span><button className="button ghost small" onClick={() => setExpanded(!expanded)}>{expanded ? "收起" : "展开高级选项"}<ChevronDown size={13} className={expanded ? "rotate" : ""} /></button></div>} />
    <div className="maintenance-summary"><CircleHelp size={16} /><p><strong>它不是第二套 Wiki 生成流程。</strong>首次使用或代码整体更新，请点击上方“生成/更新完整 Wiki”；这里只用于局部失败、提示词调整或单个模块需要重做时，选择性生成对应层级文档。</p></div>
    {expanded && <>
    ...
    <div className="generation-actions"><span>当前维护范围：{documentModeLabel(mode)}；未选择模块时处理该类型的全部模块。</span><button className="button subtle" disabled={submitting || (moduleMode && !nodes.length)} onClick={submit}>{submitting ? <LoaderCircle className="spin" size={15} /> : <RefreshCw size={15} />}重新生成所选文档</button></div>
    </>}
  </section>;
}

function Repositories({ repositories, scope, moduleTree, jobs = [], modelOptions, select, onAdd, onGenerate, onIndex, onJobs, notify, refresh }: any) {
  const selected = repositories.find((item: any) => scope?.type === "repository" && item.id === scope.id);
  const [detail, setDetail] = useState<any>(null);
  const [tree, setTree] = useState<any[]>([]);
  const latestGenerateJob = useMemo(() => jobs
    .filter((job: any) => job.kind === "generate" && job.scope_type === "repository" && job.scope_id === selected?.id)
    .sort((a: any, b: any) => Number(b.created_at || 0) - Number(a.created_at || 0))[0] || null, [jobs, selected?.id]);
  const loadDetail = useCallback(async () => {
    if (!selected) return;
    const [repository, source] = await Promise.all([
      api.get<any>(`/api/repositories/${selected.id}`),
      api.get<any>(`/api/repositories/${selected.id}/tree?kind=source`),
    ]);
    setDetail(repository);
    setTree(source.tree || []);
  }, [notify, selected?.id]);
  useEffect(() => {
    if (!selected) { setDetail(null); return; }
    loadDetail().catch((error) => notify(message(error), true));
  }, [loadDetail, notify, selected?.id]);
  useEffect(() => {
    // Jobs are polled globally every 2.5 seconds.  A status transition (most
    // importantly completed/failed after a resume) invalidates the cached run
    // list so the checkpoint button reflects the new snapshot immediately.
    if (!selected || !latestGenerateJob) return;
    loadDetail().catch((error) => notify(message(error), true));
  }, [latestGenerateJob?.id, latestGenerateJob?.status, latestGenerateJob?.finished_at, loadDetail, notify, selected?.id]);
  ...
  const resumableRun = detail?.runs?.find((run: any) => run.resumable);
  const resume = async () => { if (!selected || !resumableRun) return; try { await api.post(`/api/repositories/${selected.id}/runs/${resumableRun.id}/resume`); await refresh(); notify("已从最近断点继续生成；已完成文档不会再次调用模型。"); onJobs(); } catch (error) { notify(message(error), true); } };
  const updateConcurrency = async (value: string) => { if (!selected) return; try { const concurrency = Number(value); await api.patch<any>(`/api/repositories/${selected.id}`, { config: { module_generation_concurrency: concurrency } }); setDetail(await api.get<any>(`/api/repositories/${selected.id}`)); await refresh(); notify(`模块生成并发数已设为 ${concurrency}。下次生成生效。`); } catch (error) { notify(message(error), true); } };
  const updateModel = async (value: string) => { if (!selected || !value || value === detail?.config?.model) return; try { await api.patch<any>(`/api/repositories/${selected.id}`, { config: { model: value } }); setDetail(await api.get<any>(`/api/repositories/${selected.id}`)); await refresh(); notify(`OpenCode 模型已切换为 ${value}，下次生成或问答生效。`); } catch (error) { notify(message(error), true); } };
  return <div className="repository-layout"><section className="card repo-list-panel"><CardTitle title="本地代码仓" eyebrow={`${repositories.length} REPOSITORIES`} action={<button className="icon-button" onClick={onAdd}><Plus size={16} /></button>} /><div className="repo-cards">{repositories.map((repo: any) => <button key={repo.id} className={selected?.id === repo.id ? "selected" : ""} onClick={() => select(repo)}><span className="repo-avatar">{repo.name.slice(0, 2).toUpperCase()}</span><span><strong>{repo.name}</strong><small>{repo.path}</small></span><StatusDot status={repo.status} /></button>)}</div></section><section className="detail-column">{selected && detail ? <>
    ...
    <div className="two-column repo-details"><section className="card"><CardTitle title="源码目录" eyebrow="READ-ONLY SOURCE TREE" /><FileTree nodes={tree} /></section><section className="card"><CardTitle title="生成与环境" eyebrow="CURRENT SNAPSHOT" /><DescriptionList rows={[["Git 分支", detail.git_branch || "未检测"], ["Git 提交", detail.git_commit || "未检测"], ["当前运行", detail.active_run_id || "尚未生成"], ["叶子边界", (detail.config?.channel_module_paths || []).join(", ") || "自动识别"], ["索引状态", detail.index?.chunks ? `${detail.index.chunks} 个知识块` : "尚未索引"]]} /><label className="inline-setting"><span>OpenCode 模型</span><select value={detail.config?.model || ""} onChange={(event) => updateModel(event.target.value)}><option value="" disabled>尚未配置</option>{modelOptions.map((model: string) => <option key={model} value={model}>{model}</option>)}</select><small>列表来自本机 OpenCode；切换后不会覆盖已有 Wiki 快照，仅影响下一次生成和知识问答。</small></label><label className="inline-setting"><span>模块生成并发</span><select value={String(detail.config?.module_generation_concurrency || 2)} onChange={(event) => updateConcurrency(event.target.value)}>{[1, 2, 3, 4].map((value) => <option key={value} value={value}>{value} 个叶子任务</option>)}</select><small>仅并发独立叶子模块；父级汇总和仓库级文档按依赖顺序执行。</small></label></section></div>
...
function Wiki({ scope, onManual, notify }: { scope: Scope; onManual: () => void; notify: Function }) {
  const [docs, setDocs] = useState<any[]>([]); const [selected, setSelected] = useState<any>(null); const [filter, setFilter] = useState(""); const [publishing, setPublishing] = useState(false);
  ...
  const repositoryDocs = shown.filter((doc) => doc.kind === "manual" || !String(doc.relative_path || "").startsWith("Modules/"));
  const moduleDocs = shown.filter((doc) => doc.kind !== "manual" && String(doc.relative_path || "").startsWith("Modules/")).sort((a, b) => String(a.relative_path).localeCompare(String(b.relative_path)));
  const documentButton = (doc: any, module = false) => <button key={doc.id} className={selected?.id === doc.id ? "selected" : ""} style={module ? { paddingLeft: `${12 + documentPathDepth(doc.relative_path) * 13}px` } : undefined} onClick={() => load(doc.id)}><FileText size={15} /><span><strong>{doc.title}</strong><small>{doc.kind === "manual" ? "人工知识" : module ? `${documentRoleLabel(doc.document_role)} · ${moduleBreadcrumb(doc.relative_path)}` : `${documentRoleLabel(doc.document_role)} · ${doc.relative_path}`}</small></span>{doc.kind === "manual" && <Tags size={13} />}</button>;
  const publish = async () => {
    if (!scope || scope.type !== "repository") return;
    if (!window.confirm("将当前有效 Wiki 按原目录层级写入代码仓的 docs/clangwiki/。只更新 ClangWiki 管理的 Markdown，不修改源码，是否继续？")) return;
    setPublishing(true);
    try {
      const result = await api.post<any>(`/api/repositories/${scope.id}/documents/publish`);
      notify(`已同步 ${result.files} 篇 Markdown 到 ${result.path}`);
    } catch (error) { notify(message(error), true); }
    finally { setPublishing(false); }
  };
  if (!scope) return <ScopeEmpty />;
  return <div className="wiki-layout"><aside className="document-sidebar"><div className="document-tools"><div className="input-with-icon"><Search size={15} /><input value={filter} onChange={(e) => setFilter(e.target.value)} placeholder="筛选 Wiki 页面" /></div><button className="icon-button" onClick={onManual} title="新建人工知识页"><Plus size={16} /></button></div>{scope.type === "repository" && <div className="document-repository-actions"><button onClick={publish} disabled={publishing} title="写入代码仓 docs/clangwiki/，保持当前 Wiki 目录层级">{publishing ? <LoaderCircle className="spin" size={14} /> : <FolderGit2 size={14} />}同步到代码仓</button><a href={`/api/repositories/${scope.id}/documents/export`}><Download size={14} />导出 ZIP</a><small>同步位置：代码仓的 docs/clangwiki/</small></div>}<div className="document-list">{!!repositoryDocs.length && <div className="document-group-label"><BookOpen size={12} />仓库与事实文档</div>}{repositoryDocs.map((doc) => documentButton(doc))}{!!moduleDocs.length && <div className="document-group-label"><Layers3 size={12} />模块层级文档</div>}{moduleDocs.map((doc) => documentButton(doc, true))}{!shown.length && <Empty icon={<BookOpen />} title="还没有 Wiki" text="生成仓库或集合 Wiki 后将在这里显示。" />}</div></aside><article className="markdown-panel">{selected ? <><div className="document-meta"><div><span className="eyebrow">{selected.kind === "manual" ? "MANUAL KNOWLEDGE" : "GENERATED SNAPSHOT"}</span><h2>{selected.title}</h2><small>{selected.metadata?.module_source_path ? `对应源码层级：${selected.metadata.module_source_path}` : selected.relative_path || "人工知识页"}</small></div><div><span className="evidence-badge"><CheckCircle2 size={13} />{selected.kind === "manual" ? "工程师知识" : documentRoleLabel(selected.document_role)}</span>{selected.kind !== "manual" && <a className="button subtle small" href={`/api/wiki/documents/${encodeURIComponent(selected.id)}/export`}><Download size={13} />导出 Markdown</a>}</div></div><div className="markdown-body"><ReactMarkdown remarkPlugins={[remarkGfm]}>{selected.content || ""}</ReactMarkdown></div></> : <Empty icon={<BookOpen />} title="选择一篇文档" text="文档内容、源码证据与版本信息会显示在这里。" />}</article></div>;
}
...
function Jobs({ jobs, notify, refresh }: any) {
  ...
  const act = async (job: any, action: "cancel" | "retry" | "resume") => { try { await api.post(`/api/jobs/${job.id}/${action}`); if (action === "resume") setResumedJobIds((items) => new Set(items).add(job.id)); await refresh(); notify(action === "cancel" ? "已请求取消任务。" : action === "resume" ? "已从断点继续生成，已完成文档将被复用。" : "任务已从头重新排队。"); } catch (e) { notify(message(e), true); } };
  const recoverable = (job: any) => job.kind === "generate" && ["failed", "cancelled", "interrupted"].includes(job.status) && !job.resumed_by_job_id && !resumedJobIds.has(job.id);
  const resumed = (job: any) => Boolean(job.resumed_by_job_id) || resumedJobIds.has(job.id);
  const resumeStateLabel = (job: any) => ({ queued: "续跑中", running: "续跑中", completed: "续跑已完成", failed: "续跑失败", cancelled: "续跑已取消", interrupted: "续跑已中断" } as Record<string, string>)[job.resumed_run_status] || (resumedJobIds.has(job.id) ? "续跑中" : "已发起继续");
  return <div className="jobs-layout"><section className="card jobs-card"><CardTitle title="全部任务" eyebrow={`${jobs.length} JOBS`} /><div className="jobs-table"><div className="table-head"><span>任务</span><span>范围</span><span>状态</span><span>进度</span><span>操作</span></div>{jobs.map((job: any) => <div className={`job-line ${selected?.id === job.id ? "selected" : ""}`} key={job.id} onClick={() => loadDetail(job)}><span><JobIcon status={job.status} /><span><strong>{jobTitle(job.kind)}</strong><small>{job.message || job.id}</small></span></span><code>{job.scope_id || "系统"}</code><StatusBadge status={job.status} /><span><i className="progress"><b style={{ width: `${job.progress || 0}%` }} /></i><em>{Math.round(job.progress || 0)}%</em></span><span className="job-actions" onClick={(event) => event.stopPropagation()}>{["queued", "running"].includes(job.status) ? <button className="mini-button" onClick={() => act(job, "cancel")}>取消</button> : recoverable(job) ? <><button className="mini-button primary-mini" onClick={() => act(job, "resume")}>继续</button><button className="mini-button" onClick={() => act(job, "retry")}>重来</button></> : resumed(job) ? <span className="job-resumed">{resumeStateLabel(job)}</span> : job.status === "failed" ? <button className="mini-button" onClick={() => act(job, "retry")}>重试</button> : "—"}</span></div>)}{!jobs.length && <Empty icon={<Activity />} title="暂无任务记录" text="分析、生成和索引任务会持久化保存在这里。" />}</div></section>{selected && <aside className="card job-detail"><div className="job-detail-head"><div><span className="eyebrow">TASK DETAIL</span><h3>{jobTitle(selected.kind)}</h3><small>{selected.id}</small></div><button className="icon-button" onClick={() => { setSelected(null); setTimeline([]); }}><X size={16} /></button></div><div className="job-summary"><StatusBadge status={selected.status} /><span>{stageLabel(selected.stage)}</span><strong>{Math.round(selected.progress || 0)}%</strong></div><p className="job-message">{selected.message || "等待任务状态更新。"}</p>{recoverable(selected) ? <div className="resume-explanation"><strong>可以尝试从断点继续</strong><span>系统会复用编译分析结果和已经成功生成的模块文档，只继续剩余文档；如果代码或配置已变化，请选择重新开始。</span><div><button className="button resume-button small" onClick={() => act(selected, "resume")}><RefreshCw size={13} />从断点继续</button><button className="button ghost small" onClick={() => act(selected, "retry")}>重新开始</button></div></div> : resumed(selected) ? <div className="resume-explanation"><strong>{resumeStateLabel(selected)}</strong><span>该任务已经派生新的生成任务，仓库详情会随着续跑结果自动刷新。</span></div> : null}{selected.error && <section className="job-error"><strong><XCircle size={14} />失败原因</strong><pre>{selected.error}</pre></section>}<section className="job-timeline"><span className="eyebrow">EXECUTION TIMELINE</span>{timeline.filter((event: any) => event.type === "progress" || event.type === "status").map((event: any) => <div key={event.event_id}><i className={event.stage === "failed" ? "failed" : ""} /><span><strong>{stageLabel(event.stage)}</strong><small>{event.message}</small></span><em>{typeof event.progress === "number" ? `${event.progress}%` : ""}</em></div>)}{!timeline.length && <p>暂无阶段记录。</p>}</section></aside>}</div>;
}
...
function documentPathDepth(value: string) { return Math.max(0, String(value || "").split("/").filter(Boolean).length - 2); }
function moduleBreadcrumb(value: string) { const parts = String(value || "").split("/").filter(Boolean); return parts.slice(1, -1).join(" › ") || "根模块"; }
